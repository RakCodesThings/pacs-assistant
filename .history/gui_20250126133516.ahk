...
class KeybindGUI {
    ...
    ListenForKeys(ih, funcName) {
        ...
        ; Only process if we have an EndKey or if all modifier keys were released
        if ih.EndKey {
            modifiers := ""
            modifiers .= hasCtrl ? "^" : ""
            modifiers .= hasAlt ? "!" : ""
            ...
            ; If EndKey is a modifier key and that's the only key pressed, bind just that key
            if (ih.EndKey = "Ctrl" && hasCtrl && !hasAlt && !hasShift && !hasWin) 
                || (ih.EndKey = "Alt" && hasAlt && !hasCtrl && !hasShift && !hasWin)
                || (ih.EndKey = "Shift" && hasShift && !hasCtrl && !hasAlt && !hasWin)
                || ((ih.EndKey = "LWin" || ih.EndKey = "RWin") && hasWin && !hasCtrl && !hasAlt && !hasShift) {
                newBind := ih.EndKey
            } else {
                newBind := modifiers ih.EndKey
            }
