...
class KeybindGUI {
    ...
    __New() {
        ...
        KeybindGUI.funcMap["testFunction"] := KeybindGUI.TestFunc
        KeybindGUI.funcMap["toggleDictation"] := KeybindGUI.ToggleFunc
    ...
    ApplyBinds() {
        ...
        for funcName, hk in activeHotkeys {
            try {
                Hotkey(hk, "Off")
            } catch as err {
                MsgBox("Error disabling hotkey: " hk "`nError: " err.Message)
            }
        }
        activeHotkeys := Map()
        
        for funcName, bind in profiles[currentProfile] {
            Hotkey(bind, KeybindGUI.funcMap[funcName])
        }
    }
