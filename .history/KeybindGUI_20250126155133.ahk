...
class KeybindGUI {
    ...
    ShowAddFunctionDialog(listView) {
        ...
        for funcName, _ in ProfileManager.availableFunctions {
            if !ProfileManager.profiles[ProfileManager.currentProfile].Has(funcName) {
                if (funcName != "Custom Keybind") {  ; Don't show Custom Keybind in regular list
        ...
        selectorGui.Add("Button", "w200", "Add Custom Keybind").OnEvent("Click", (*) => (selectorGui.Destroy(), this.ShowCustomKeybindDialog(listView)))
        
        selectorGui.Add("Text", "xm y+10", "Or select a predefined function:")
    ...
    ResizeColumns(listView) {
        listView.ModifyCol(1, "AutoHdr")  ; Function column - size to fit both content and header
        listView.ModifyCol(2, "AutoHdr")  ; Keybind column - size to fit both content and header
    }
} 