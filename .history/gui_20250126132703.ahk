...
class KeybindGUI {
    ...
    static defaultBinds := Map("toggleDictation", "F16")
    ...
    LoadProfiles() {
        try {
            Loop Files "profiles/*.ini" {
                ...
                KeybindGUI.profiles[profileName] := this.LoadProfile(A_LoopFilePath)
            }
        }
    }
    ...
    CreateProfile(name, inputGui) {
        if name != "" {
            this.profiles[name] := this.defaultBinds.Clone()
            this.SaveProfile(name, this.profiles[name])
    ...
    ShowProfileSelector() {
        ...
        lb := selectorGui.Add("ListBox", "w200", this.profiles.Keys())
    ...
    CreateMainGUI() {
        this.gui := Gui("+AlwaysOnTop", "PACS Assistant - " this.currentProfile)
        this.gui.Add("Text",, "Current Profile: " this.currentProfile)
        
        this.gui.Add("Text", "xm y+20", "Keybinds:")
        y := 70
        
        for funcName, bind in this.profiles[this.currentProfile] {
    ...
    ListenForKeys(ih, funcName) {
        ...
        if GetKeyState("Escape") {
            this.StopListening()
            this.listeningControl.Value := this.profiles[this.currentProfile][funcName]
            return
        }
        ...
        if keys.Length > 0 {
            ...
            this.profiles[this.currentProfile][funcName] := newBind
    ...
    SaveCurrentProfile() {
        this.SaveProfile(this.currentProfile, this.profiles[this.currentProfile])
