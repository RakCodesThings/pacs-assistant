...
global defaultBinds := Map(
    "toggleDictation", "F16",
    "testFunction", "^!x"  ; Add the new function with Ctrl+Alt+X default
)
global activeHotkeys := Map()
global isListening := false
global listeningControl := ""

; Global function to handle hotkey calls
callFunction(fname, *) {
    if (fname = "testFunction")
        KeybindGUI.testFunction()
    else if (fname = "toggleDictation")
        KeybindGUI.toggleDictation()
}
...
class KeybindGUI {
    ...
    ApplyBinds() {
        ...
        for funcName, bind in profiles[currentProfile] {
            try {
                ...
                if (funcName = "testFunction")
                    Hotkey(bind, KeybindGUI.testFunction)
                else if (funcName = "toggleDictation")
                    Hotkey(bind, KeybindGUI.toggleDictation)
    ...
    static toggleDictation(*) {
        MsgBox("Toggling Dictation", "Dictation")
    }

    static testFunction(*) {
        MsgBox("Hello", "Test Function")
    }

    static HandleHotkey(funcName, *) {
        if (funcName = "testFunction")
            KeybindGUI.testFunction()
        else if (funcName = "toggleDictation")
            KeybindGUI.toggleDictation()
