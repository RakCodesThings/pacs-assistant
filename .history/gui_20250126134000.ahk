...
class KeybindGUI {
    ...
    PrettifyHotkey(hotkeyStr) {
        ...
        pretty := hotkeyStr
        pretty := StrReplace(pretty, "^", "Ctrl + ")
        pretty := StrReplace(pretty, "!", "Alt + ")
        pretty := StrReplace(pretty, "+", "Shift + ")
        pretty := StrReplace(pretty, "#", "Win + ")
        
        ; Split into parts
        parts := StrSplit(pretty, "+")
        
        ; Trim all parts and capitalize the last part (the actual key)
        for i, part in parts {
            parts[i] := Trim(part)
            if (i = parts.Length)  ; Last part (the key)
                parts[i] := Format("{:U}", parts[i])
        }
