...
class KeybindGUI {
    ...
    CreateMainGUI() {
        ...
        for funcName, bind in currentProfile.binds {
            ; Determine if this is a custom or built-in function
            funcType := currentProfile.customFuncs.Has(funcName) ? "Custom" : "Built-in"
    ...
    ShowAddFunctionDialog(listView) {
        ...
        ; Add custom functions from current profile that aren't bound
    ...
    DeleteCustomFunction(funcName, selectorGui) {
        ...
        if (MsgBox("Are you sure you want to delete the custom function '" funcName "'?", "Confirm Delete", "YesNo Icon!") = "Yes") {
            ...
            ProfileManager.profiles[ProfileManager.currentProfile].customFuncs.Delete(funcName)
            
            ; Remove from current profile's bindings
            ProfileManager.profiles[ProfileManager.currentProfile].binds.Delete(funcName)
            
            ; Save the current profile
            ProfileManager.SaveProfile(
                ProfileManager.currentProfile, 
                ProfileManager.profiles[ProfileManager.currentProfile].binds,
                ProfileManager.profiles[ProfileManager.currentProfile].customFuncs
            )
    ...
    AddCustomKeybind(name, keys, window, listView, customGui) {
        ...
        ; Add to ListView
        listView.Add(, funcName, "Unassigned", "Custom")
    ...
    AddFunction(funcName, listView, selectorGui) {
        ...
        ; Add to ListView
        funcType := ProfileManager.profiles[ProfileManager.currentProfile].customFuncs.Has(funcName) ? "Custom" : "Built-in"
        listView.Add(, funcName, "Unassigned", funcType)
        this.ResizeColumns(listView)
