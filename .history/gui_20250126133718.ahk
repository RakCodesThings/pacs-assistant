...
class KeybindGUI {
    ...
    StartListening(control, funcName, *) {
        ...
        ih.OnKeyDown := this.OnKeyDown.Bind(this, funcName, control)
        ih.Start()
    }
    
    OnKeyDown(funcName, control, ih, vk, sc) {
        ...
        key := GetKeyName("vk" vk)
        
        ; Handle Escape to cancel
        if (key = "Escape") {
            this.StopListening()
            control.Value := profiles[currentProfile][funcName]
            return
        }
